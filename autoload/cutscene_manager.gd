## CutsceneManager - Handles story cutscenes and scripted sequences (Autoload)
extends Node
...
func play_cutscene(cutscene_id: String) -> void:
	...
	if player_reference and player_reference.has_method("set_cutscene_mode"):
		player_reference.set_cutscene_mode(true)
...
func end_cutscene() -> void:
	...
	if player_reference and player_reference.has_method("set_cutscene_mode"):
		player_reference.set_cutscene_mode(false)
...
func _play_summoning_cutscene() -> void:
	...
	# Show dialogue
	await _show_dialogue("Crazy Joe", "By the ancient pacts and desperate pleas, I summon champions from beyond the veil!")
	
	# Flash effect for summoning
	_flash_screen(Color.CYAN, 0.5)
	await cutscene_timer.timeout
	
	await _show_dialogue("Narrator", "A blinding light fills the grand hall of the Mage Association...")
	
	await _show_dialogue("Crazy Joe", "Yes! It worked! Welcome, mighty heroes! You must save our realm from—")
	
	await _show_dialogue("Mage Elder", "Joe! What have you done?! These... these are mere weaklings!")
	
	await _show_dialogue("Crazy Joe", "No, no! They have potential! They can grow stronger than any of us!")
	
	# Give starting items and update quest
	if player_reference and player_reference.has_method("add_gold"):
		player_reference.add_gold(50)
...
func _play_expulsion_cutscene() -> void:
	...
	await _show_dialogue("Mage Elder", "Crazy Joe, your recklessness could have torn a new rift! You are hereby expelled from the Association!")
	
	await _show_dialogue("Crazy Joe", "But... but the realm needs—")
	
	await _show_dialogue("Mage Elder", "SILENCE! Guards, escort him out. Strip him of his robes and staff.")
	
	await _show_dialogue("Mage Elder", "As for these... 'Summoned'... Give them the bare minimum. Some gold and that old tree no one wants.")
	
	await _show_dialogue("Association Clerk", "Here. 50 gold coins and residency papers for the Old Oak. Try not to die.")
...
func _play_bob_entrance_cutscene() -> void:
	...
	await _show_dialogue("Narrator", "That night, as you settle into the dusty interior of the Old Oak...")
	...
	await _show_dialogue("???", "Well, well. Squatters in MY tree.")
	
	await _show_dialogue("Bob", "Name's Bob. I've been living here for decades. Just made it LOOK abandoned.")
	
	await _show_dialogue("Bob", "You're those Summoned everyone's talking about. Weak, they say. But I sense... potential.")
	
	await _show_dialogue("Bob", "I've been investigating the Dungeon Fracture. Something's off about the landfill outside town.")
	
	await _show_dialogue("Bob", "Prove yourselves. Clear out whatever's lurking there and bring back samples.")
...
func _play_crystal_creation_cutscene() -> void:
	await _show_dialogue("Bob", "Interesting... Joe's trash contains fragments of spatial magic.")
	
	# Crafting effect
	_flash_screen(Color.BLUE, 0.5)
	await cutscene_timer.timeout
	
	await _show_dialogue("Bob", "By fusing these fragments... There! A Fast Travel Crystal.")
	
	await _show_dialogue("Bob", "This will let you teleport between attuned locations. Very useful for your journey.")
	
	await _show_dialogue("Bob", "The real quest begins now. The Fracture's cause is out there somewhere.")
	
	await _show_dialogue("Bob", "Explore, grow stronger, and find the truth. The realm's fate depends on it.")
...
func _show_dialogue(speaker: String, text: String, duration: float = 3.0) -> void:
	# Show dialogue using UI or print to console
...
func _spawn_bob_at_tree() -> void:
	# This will be called to spawn Bob NPC in the home tree
	# The actual implementation depends on the scene structure
	print("Bob appears in the Home Tree!")
