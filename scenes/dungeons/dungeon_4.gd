...
var dungeon_id: String = "dungeon_4"
var dungeon_name: String = "Shadow Citadel"
var difficulty: int = 4
var max_enemies: int = 20  # Scaled from template
...
func track_existing_enemies() -> void:
	...
	# Dungeon 4 has the most challenging pre-placed enemies
	var enemy_nodes = [
		$EnemyRoom1/Enemy1,
		$EnemyRoom1/Enemy2,
		$EnemyRoom2/Enemy1,
...
func spawn_random_enemies(count: int) -> void:
	...
	for i in range(min(count, enemy_spawn_points.size())):
		...
		# Select enemy type based on dungeon 4 pool (elite enemies)
...
func spawn_boss() -> void:
	"""Spawn the dungeon 4 boss - Shadow Lord"""
	print("Spawning Shadow Lord Boss!")

	var boss = $BossArea/ShadowLord
	if boss:
		enemies.append(boss)
		current_enemies += 1
...
func create_completion_effect() -> void:
	...
	# Spawn shadow particle effect for dungeon 4 theme
	...
	material.color = Color(0.1, 0.0, 0.2)  # Deep shadow purple
...
func _on_boss_died(_enemy: Node) -> void:
	...
	boss_defeated.emit()
	print("Shadow Lord defeated! Treasure room unlocked.")
	if QuestManager:
		QuestManager.report_kill(Enums.EnemyType.BOSS, "shadow_lord")
